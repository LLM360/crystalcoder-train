...
def pretrain(train_valid_test_dataset_provider,
    ...
    norm_modules_name_patterns = (
        "norm",
    )
    embedding_modules_name_patterns = (
        "embedding",
    )
    def no_wd_decay_cond (name, param):
        # no weight decay for norm
        
        for pattern in norm_modules_name_patterns:
            if pattern in name:
                return True
    ...
    def scale_lr_cond(name, param):
        ...
        for pattern in embedding_modules_name_patterns:
            if pattern in name:
                return False
        for pattern in norm_modules_name_patterns:
            if pattern in name:
                return False
        return True # scale lr for all other params
    
    if args.use_mup:
        lr_mult = args.mup_width_scale
    else:
        lr_mult = 1.0
    model, optimizer, opt_param_scheduler = setup_model_and_optimizer(
        model_provider, model_type, 
        no_wd_decay_cond=no_wd_decay_cond, scale_lr_cond=scale_lr_cond, lr_mult=lr_mult
    )
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        log_string += ' learning rate: {:.10E} |'.format(learning_rate)
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        report_memory_flag = training_log(loss_dict, total_loss_dict,
                                          optimizer.param_groups[0]['lr']/optimizer.param_groups[0].get('lr_mult', 1.0),
