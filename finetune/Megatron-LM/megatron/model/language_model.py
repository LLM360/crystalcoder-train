...
def get_language_model_mup(config, num_tokentypes, add_pooler,
                       encoder_attn_mask_type,
                       add_encoder=True,
                       add_decoder=False,
                       decoder_attn_mask_type=AttnMaskType.causal,
    ...
    # Language model.
    language_model = TransformerLanguageModelWithMuP(
        config,
        encoder_attn_mask_type,
        num_tokentypes=num_tokentypes,
        add_encoder=add_encoder,
...
# muP version of TransformerLanguageModel


class TransformerLanguageModelWithMuP(TransformerLanguageModel):
    """Transformer language model.
    ...
    def __init__(self,
        ...
        super(TransformerLanguageModelWithMuP, self).__init__(
            config=config,
    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
        ...
        if self.pre_process:
            encoder_input = self.embedding(enc_input_ids, enc_position_ids,
                                           tokentype_ids=tokentype_ids)
            encoder_input = self.mup_embeddings_scale * encoder_input
        ...
        if self.add_retriever and self.pre_process:
            retriever_input = self.embedding(retriever_input_ids,
                                             retriever_position_ids,
                                             tokentype_ids=tokentype_ids)
            raise NotImplementedError # maybe should scale
        else:
            retriever_input = None
